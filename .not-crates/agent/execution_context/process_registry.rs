...
/// agent — all running processes receive `SIGTERM` (Unix) or are forcibly
/// killed (non-Unix) on `shutdown_all`.
...
impl ProcessRegistry {
    ...
    /// is not recognised. On non-Unix targets, only `SIGKILL` (force kill) is
    /// supported.
...
/// Send `signal` to `pid`. On non-Unix targets only SIGKILL is meaningful;
/// all other signal names are silently treated as a force kill.
fn terminate_pid(pid: u32, signal: &str) {
    #[cfg(unix)]
    {

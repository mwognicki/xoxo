...
pub(crate) fn inspect_matlab_structure(content: &str) -> Result<CodeStructure, CodeStructureError> {
...
mod tests {
    ...
    #[test]
    fn inspects_matlab_items() {
        let structure = inspect_matlab_structure("function y = boot(x)\ny = x + 1;\nend\n").unwrap();
        let items = structure
            .items
            .iter()

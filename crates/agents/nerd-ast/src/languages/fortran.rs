...
pub(crate) fn inspect_fortran_structure(content: &str) -> Result<CodeStructure, CodeStructureError> {
...
fn classify_node(node: Node<'_>) -> Option<CodeItemKind> {
    match node.kind() {
        "function_statement" | "function_subprogram" => Some(CodeItemKind::Function),
        "subroutine_statement" | "subroutine_subprogram" => Some(CodeItemKind::Function),
        "derived_type_definition" | "derived_type_statement" => Some(CodeItemKind::Struct),
        "interface_block" | "interface_statement" => Some(CodeItemKind::Trait),
        _ => None,
    }
}
...
mod tests {
    ...
    #[test]
    fn inspects_fortran_items() {
        let structure = inspect_fortran_structure(
        ...
        assert_eq!(
            items,
            vec![(CodeItemKind::Function, Some("boot"))]
        );

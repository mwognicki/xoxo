...
pub(crate) fn inspect_erlang_structure(content: &str) -> Result<CodeStructure, CodeStructureError> {
...
fn classify_node(node: Node<'_>) -> Option<CodeItemKind> {
    match node.kind() {
        "module_attribute" => Some(CodeItemKind::Module),
        "function" | "function_clause" => Some(CodeItemKind::Function),
        "record_decl" => Some(CodeItemKind::Struct),
        _ => None,
    }
}
...
fn node_name(node: Node<'_>, source: &[u8]) -> Option<String> {
    node.child_by_field_name("name")
        .or_else(|| first_atom(node))
        .and_then(|name| name.utf8_text(source).ok().map(str::to_string))
}

fn first_atom(node: Node<'_>) -> Option<Node<'_>> {
    if matches!(node.kind(), "atom" | "variable") {
        return Some(node);
    }

    let mut cursor = node.walk();
    for child in node.children(&mut cursor) {
        if let Some(atom) = first_atom(child) {
            return Some(atom);
        }
    }
    None
...
mod tests {
    ...
    #[test]
    fn inspects_erlang_items() {
        let structure = inspect_erlang_structure("-module(app).\nboot() -> ok.\n").unwrap();
        let items = structure
            .items
            .iter()

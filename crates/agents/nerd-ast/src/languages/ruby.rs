...
pub(crate) fn inspect_ruby_structure(content: &str) -> Result<CodeStructure, CodeStructureError> {
...
fn collect_items(node: Node<'_>, source: &[u8], items: &mut Vec<CodeItem>) {
    if let Some(kind) = classify_node(node) {
        ...
        if name.is_none() && matches!(kind, CodeItemKind::Module | CodeItemKind::Struct) {
...
fn classify_node(node: Node<'_>) -> Option<CodeItemKind> {
    match node.kind() {
        "class" => Some(CodeItemKind::Struct),
        "module" => Some(CodeItemKind::Module),
        "method" | "singleton_method" => Some(CodeItemKind::Method),
        _ => None,
    }
}
...
mod tests {
    ...
    #[test]
    fn inspects_ruby_items_in_source_order() {
        let structure = inspect_ruby_structure(
            "module App\n\
             \x20\x20class User\n\
             \x20\x20\x20\x20def greet\n\
             \x20\x20\x20\x20\x20\x20'hi'\n\
             \x20\x20\x20\x20end\n\
             \x20\x20end\n\
             end\n",
        )

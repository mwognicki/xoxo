...
pub(crate) fn inspect_pascal_structure(content: &str) -> Result<CodeStructure, CodeStructureError> {
...
fn classify_node(node: Node<'_>) -> Option<CodeItemKind> {
    match node.kind() {
        "program" | "unit" => Some(CodeItemKind::Module),
        "procExternal" => Some(CodeItemKind::Function),
        "typeDecl" | "recType" => Some(CodeItemKind::Struct),
        "interface" => Some(CodeItemKind::Trait),
        _ => None,
    }
}
...
mod tests {
    ...
    #[test]
    fn inspects_pascal_items() {
        let structure = inspect_pascal_structure(

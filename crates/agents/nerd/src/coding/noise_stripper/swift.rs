//! Swift-specific noise stripping.
...
#[cfg(test)]
mod tests {
    use super::strip_swift_noise;

    #[test]
    fn strips_line_comments() {
        let input = "let value = 1 // comment\n";
        let output = strip_swift_noise(input);
        assert_eq!(output, "let value = 1 //        \n");
    }

    #[test]
    fn keeps_comment_markers_inside_multiline_strings() {
        let input = "let text = \"\"\"// not comment\n/* nope */\"\"\"\n";
        let output = strip_swift_noise(input);
